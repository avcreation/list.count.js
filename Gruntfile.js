module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    shell: {
      ...
      standalone: {
        command: 'component build --standalone ListCount -n list.count.standalone'
      },
      mkdir: {
        command: 'mkdir -p dist'
      },
      move: {
        command: 'mv build/list.count.standalone.js dist/list.count.js'
      },
      remove: {
    ...
    uglify: {
      target: {
        files: {
          'dist/list.count.min.js': ['dist/list.count.js']
        }
      }
    },
    mocha: {
